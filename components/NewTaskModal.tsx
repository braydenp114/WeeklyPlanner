...
function InlineDropdown({
  ...
  return (
    <View style={{ position: "relative", zIndex: open ? 100 : 1 }}>
      ...
      >
        <Text
          style={[inlineStyles.dropLabel, { color: theme.text }]}
          numberOfLines={1}
        >
          {selectedLabel}
        </Text>
      ...
      {open && (
        ...
        >
          {options.map((opt) => (
            ...
            >
              <Text style={[inlineStyles.dropItemText, { color: theme.text }]}>
                {opt.label}
              </Text>
            </TouchableOpacity>
          ))}
...
export default function NewTaskModal({
  ...
  const [customRecurrenceRule, setCustomRecurrenceRule] =
    useState<CustomRecurrenceRule | null>(null);
  ...
  const [deadlineWarningHours, setDeadlineWarningHours] = useState("72");
  const [linkedDeadlineId, setLinkedDeadlineId] = useState<string | null>(null);
  ...
  const handleChecklistItemTextChange = useCallback(
  ...
  // ── Anchor positions ──
  const [startDateAnchor, setStartDateAnchor] = useState<AnchorRect | null>(
    null,
  );
  const [endDateAnchor, setEndDateAnchor] = useState<AnchorRect | null>(null);
  const [startTimeAnchor, setStartTimeAnchor] = useState<AnchorRect | null>(
    null,
  );
  ...
  useEffect(() => {
    if (visible) {
      getUpcomingDeadlines()
        .then((deadlines) =>
          setAvailableDeadlines(
            deadlines.map((d) => ({ id: d.id!, title: d.title })),
          ),
        )
        .catch(() => setAvailableDeadlines([]));
      ...
      if (editTaskData) {
        ...
        let customLbl = "";
        if (
          editTaskData.recurrence === "custom" &&
          editTaskData.customRecurrenceRule
        ) {
          ...
          if (rule.unit === "week" && rule.daysOfWeek) {
            const dayNames = rule.daysOfWeek.map(
        ...
        if (editTaskData.latitude && editTaskData.longitude) {
          setLocationCoordinates({
            ...
            longitude: editTaskData.longitude,
          });
        } else {
        ...
        setNotifications(
          editTaskData.notification ? [editTaskData.notification] : [],
        );
        setColorHex(editTaskData.colorHex);
        if (editTaskData.category) {
          setCategory(editTaskData.category);
          setCategoryOptions((prev) =>
            prev.includes(editTaskData.category!)
              ? prev
              : [...prev, editTaskData.category!],
          );
        } else {
          setCategory("Study");
        }
        ...
        setIsDeadline(!!editTaskData.isDeadline);
        setDeadlineWarningHours(
          String(editTaskData.deadlineWarningHours ?? 72),
        );
        setLinkedDeadlineId(editTaskData.linkedDeadlineId ?? null);
      } else {
        // Default new task
        ...
        setChecklistItems([]);
        setIsDeadline(false);
        setDeadlineWarningHours("72");
        setLinkedDeadlineId(null);
      }
  ...
  const recurrenceOptions = useMemo(() => {
    ...
    return [
      ...
      {
        label:
          recurrence === "custom" && customRuleLabel
            ? customRuleLabel
            : "Custom…",
        value: "custom",
      },
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const trimmedChecklistItems = checklistItems.filter(
      ...
      const taskData: CreateTaskData = {
        ...
        customRecurrenceRule:
          recurrence === "custom" ? customRecurrenceRule : null,
        ...
        isDeadline,
        deadlineWarningHours: isDeadline
          ? parseInt(deadlineWarningHours, 10) || 72
          : null,
            ...
            >
                  ...
                  <View style={styles.dateTimeRow}>
                    ...
                    {!allDay && (
                      <>
                        ...
                        <Text
                          style={[styles.toLabel, { color: theme.textMuted }]}
                        >
                          to
                        </Text>
                        <TouchableOpacity
                        ...
                        {NOTIFICATION_OPTIONS.map((opt) => (
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              setNotifications((prev) => [
                                ...prev,
                                { type: opt.label, minutesBefore: opt.minutes },
                              ]);
                              setNotifPickerOpen(false);
                            }}
                          >
              ...
              <View style={[styles.fieldSection, { zIndex: 10 }]}>
                ...
                <View style={{ position: "relative", flex: 1 }}>
                  ...
                  >
                    ...
                    >
                      {COLOR_OPTIONS.find((c) => c.hex === colorHex)?.name ||
                        "Color"}
                    </Text>
                    <MaterialIcons
                      name="arrow-drop-down"
              ...
              >
                <View style={styles.descToolbar}>
                  <TouchableOpacity
                    style={[
                      styles.descToolBtn,
                      descBold && { backgroundColor: theme.surfaceContainer },
                    ]}
                  ...
                  <TouchableOpacity
                    style={[
                      styles.descToolBtn,
                      descItalic && { backgroundColor: theme.surfaceContainer },
                    ]}
              ...
              >
                <View style={styles.checklistToggleRow}>
                  ...
                  <Text
                    style={[styles.fieldLabel, { color: theme.text, flex: 1 }]}
                  >
                    Checklist
                  </Text>
                  <Switch
                    ...
                    {checklistItems.map((item) => (
                      <View key={item.id} style={styles.checklistItemRow}>
                        ...
                        <TextInput
                          ...
                          value={item.text}
                          onChangeText={(text) =>
                            handleChecklistItemTextChange(item.id, text)
                          }
              ...
              >
                ...
                {isDeadline && (
                  <View style={{ marginTop: 10, marginLeft: 36 }}>
                    <Text
                      style={[styles.miniLabel, { color: theme.textMuted }]}
                    >
                      WARN ME (HOURS BEFORE)
                    </Text>
                    <TextInput
                      style={[
                        inlineStyles.dropBtn,
                      ...
                      onChangeText={setDeadlineWarningHours}
              ...
              {/* Link to deadline */}
              {!isDeadline && availableDeadlines.length > 0 && (
                <View style={[styles.fieldSection, { zIndex: 8 }]}>
                  <View style={styles.fieldIcon}>
                    <MaterialIcons
                      name="link"
                      size={20}
                  ...
                  <View style={{ position: "relative", flex: 1 }}>
                    <TouchableOpacity
                      ...
                      onPress={() => setDeadlinePickerOpen(!deadlinePickerOpen)}
                    >
                      <Text
                        style={[
                          inlineStyles.dropLabel,
                      ...
                      >
                        {linkedDeadlineId
                          ? availableDeadlines.find(
                              (d) => d.id === linkedDeadlineId,
                            )?.title || "Linked deadline"
                          : "Link to a deadline (optional)"}
                      </Text>
                      <MaterialIcons
                        name="arrow-drop-down"
                        size={18}
                    ...
                    {deadlinePickerOpen && (
                      ...
                      >
                        ...
                        {availableDeadlines.map((d) => (
                          <TouchableOpacity
                            key={d.id}
                            style={inlineStyles.dropItem}
                            onPress={() => {
                              setLinkedDeadlineId(d.id);
                              setDeadlinePickerOpen(false);
                            }}
                          >
                            <Text
